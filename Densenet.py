...
class DenseNet():
    ...
    def Dense_net(self, input_x):
        ...
        for i in range(self.nb_blocks) :
            # 6 -> 12 -> 48
            x = self.dense_block(input_x=x, nb_layers=12, layer_name='dense_'+str(i))
...
with tf.Session() as sess:
    ...
    for epoch in range(total_epochs):
        ...
        total_batch = int(train_size * 3 / batch_size)
        ...
        for step in range(total_batch):
            ...
            if step % 20 == 0:
                # test_time = time.time()
                test_images, test_labels = input_data.test_next_batch(filepath)
                test_feed_dict = {
                    x: test_images,
                    label: test_labels,
                    learning_rate: epoch_learning_rate,
